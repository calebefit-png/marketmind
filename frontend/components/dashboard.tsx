...
import { assets, getAssetsByClass } from "@/lib/portal-data";
...
export function Dashboard() {
  ...
  const topAssets = assets.filter((asset) => ["acoes", "fiis", "etfs", "bdrs", "stocks", "cripto"].includes(asset.assetClass)).slice(0, 8);
  ...
  return <PortalShell>
    ...
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5">
      ...
      <MetricCard label="Ibovespa" value="166.934,20" delta={-0.10} detail="Índice Brasil" />
      <MetricCard label="Dólar" value="R$ 5,22" delta={0.73} detail="Câmbio comercial" />
      <MetricCard label="IFIX" value="3.686,33" delta={-0.17} detail="Fundos imobiliários" />
    </section>
    ...
    <section className="mt-8"><SectionTitle title="Ativos em evidência" subtitle="Uma visão transversal das classes cobertas no terminal." href="/rankings" linkLabel="Explorar rankings" /><AssetTable assets={topAssets} description="Os dados B3 e internacionais abaixo são demonstrativos até a integração de uma fonte verificável." compact /></section>

    <section className="mt-8 grid gap-5 xl:grid-cols-3"><MiniRanking title="Maiores dividendos" assets={[...getAssetsByClass("acoes"), ...getAssetsByClass("fiis")]} metric="dy" href="/rankings" /><MiniRanking title="Movimentação do dia" assets={assets.filter((asset) => asset.change !== 0)} metric="change" href="/rankings" /><article className="rounded-xl border border-cyan-400/15 bg-gradient-to-br from-cyan-400/[0.11] to-slate-900/35 p-5"><Sparkles className="text-cyan-300" size={20} /><h2 className="mt-5 text-lg font-semibold tracking-[-0.025em] text-white">Análise antes da decisão.</h2><p className="mt-2 text-sm leading-relaxed text-slate-400">Compare fundamentos, rastreie filtros, acompanhe sua carteira e saiba quando os dados são reais, oficiais ou demonstrativos.</p><div className="mt-5 grid gap-2"><Link href="/comparador" className="flex items-center justify-between rounded-lg bg-slate-950/55 px-3 py-2.5 text-sm font-medium text-slate-200 transition hover:bg-slate-950">Comparar ativos <ArrowRight size={15} /></Link><Link href="/carteira" className="flex items-center justify-between rounded-lg bg-slate-950/55 px-3 py-2.5 text-sm font-medium text-slate-200 transition hover:bg-slate-950">Acompanhar carteira <ArrowRight size={15} /></Link></div></article></section>

    <section className="mt-8 rounded-xl border border-slate-800 bg-slate-900/35 p-5"><div className="flex items-start gap-3"><Landmark className="mt-0.5 text-sky-300" size={18} /><div><h2 className="text-sm font-semibold text-slate-100">Transparência de dados</h2><p className="mt-1 max-w-4xl text-xs leading-relaxed text-slate-500">BTC/USDT utiliza o streaming já conectado ao MarketMind e a Selic usa a série pública configurada. Cotações de B3, FIIs, exterior, ETFs, renda fixa, moedas e commodities ainda são dados demonstrativos nesta interface e aparecem com selo âmbar até a ativação de conectores verificáveis.</p></div></div></section>
  </PortalShell>;
}
